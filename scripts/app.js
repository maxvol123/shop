...
function lazy(params) {
    ...
    const lazyImage = document.querySelectorAll('.foto')
    ...
    function lazyScroll() {
        if (document.querySelectorAll('.foto').length>0) {
            lazyScrollCheck()
        }
    }
    ...
    function lazyScrollCheck() {
        let imgIndex = lazyImagePosition.findIndex(
            item => pageYOffset > item-windowHigh
        )
        if (imgIndex>=0) {
